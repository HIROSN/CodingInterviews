...
void Make(vector<Entry> entries, string target)
{
    ...
    for (vector<Entry>::size_type i = 0; i < entries.size(); i++)
    {
        if (entries[i].m_target == target)
        {
            ...
            if (!Exists(entries[i].m_target))
            {
                // First make all dependencies.
                for (vector<Entry>::size_type j = 0;
                    j < entries[i].m_deps.size();
                    j++)
                {
                    Make(entries, entries[i].m_deps[j].m_target);
                }

                // Make this target.
                if (entries[i].m_command.length() > 0)
                {
                    ExecuteCommand(entries[i].m_command);
                }
            }
